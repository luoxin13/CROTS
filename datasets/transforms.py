...
def crop(img, mask, sizew, sizeh):
    # padding height or width if smaller than cropping size
    w, h = img.size
    padw = sizew - w if w < sizew else 0
    padh = sizeh - h if h < sizeh else 0
    img = ImageOps.expand(img, border=(0, 0, padw, padh), fill=0)
    mask = ImageOps.expand(mask, border=(0, 0, padw, padh), fill=255)

    # cropping
    w, h = img.size
    x = random.randint(0, w - sizew)
    y = random.randint(0, h - sizeh)
    img = img.crop((x, y, x + sizew, y + sizeh))
    mask = mask.crop((x, y, x + sizew, y + sizeh))
